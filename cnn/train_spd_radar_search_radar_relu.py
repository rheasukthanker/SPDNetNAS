...
from model_spd_v0_radar_relu_search import Network
...
def use_relu_as_convex_layer(x, n):
    x_ = cp.Parameter(n)
    y_ = cp.Variable(n)
    # define the objective and constraint
    objective = cp.Minimize(cp.sum_squares(y_ - x_))
    constraint = [y_ >= 0, cp.sum(y_) == 1.]
    optimization_problem = cp.Problem(objective, constraint)
    # python interface
    layer = CvxpyLayer(optimization_problem, parameters=[x_], variables=[y_])
    y, = layer(x)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print(use_relu_as_convex_layer(model.alphas_normal, n))
        print(use_relu_as_convex_layer(model.alphas_reduce, n))
        print(
            torch.sum(use_relu_as_convex_layer(model.alphas_normal, n), dim=1))
        print(
            torch.sum(use_relu_as_convex_layer(model.alphas_reduce, n), dim=1))
        # training

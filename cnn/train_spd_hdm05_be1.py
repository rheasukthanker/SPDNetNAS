...
from model_spd_v0_hdm05_be1 import Network
import random
import torch as th
import genotypes
...
class DatasetHDM05(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        input = x
        aug = True
        s = np.random.binomial(1, 0.1, 1)
        #input=input.unsqueeze(0)
        if aug == True and s == 1 and self.val == False:
            input = input.unsqueeze(0)
            alpha = float(np.random.uniform(low=0.001, high=0.002, size=(1, )))
            lam = alpha * torch.einsum('bii->b', input.squeeze(1))
            lam = lam.view(lam.shape[0], 1, 1)
            input = input.squeeze(1) + lam * torch.eye(input.shape[2]).repeat(
                lam.shape[0], 1, 1)
...
def main(trainperc):
    ...
    model = Network(1, 93, 30, num_classes, 2, genotype)
    model = model.double()
    p = model.state_dict()

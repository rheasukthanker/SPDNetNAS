...
from model_spd_v0_be1 import Network
import random
import torch as th
import genotypes
...
class DatasetRadar(data.Dataset):
    def __init__(self, path, names, val):
        self._path = path
        self._names = names
        self.val = val
    ...
    def __getitem__(self, item):
        ...
        #if aug==True and self.val==False:
        #    alpha=float(np.random.uniform(low=0.001, high=0.01, size=(1,)))
        #    lam = alpha * torch.einsum('bii->b', input.squeeze(1))
        #    lam = lam.view(lam.shape[0], 1, 1)
        #    input= input.squeeze(1) + lam * torch.eye(input.shape[2]).repeat(lam.shape[0], 1, 1)
...
class DataLoaderRadar:
    def __init__(self, data_path, pval, batch_size):
        ...
        train_set = DatasetRadar(
            ...
            False)
        test_set = DatasetRadar(data_path, names[:N_test], True)
        val_set = DatasetRadar(data_path, names[N_test:N_test + N_val], True)
...
def main(trainperc):
    ...
    optimizer.lr = args.learning_rate * 0.99
    ...
    for epoch in range(args.epochs):
        #scheduler.step()
        logging.info('epoch %d lr %e', epoch, optimizer.lr)
...
def train(train_queue, model, criterion, optimizer):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        for name, param in model.named_parameters():
            if param.grad == None:
                param.grad = param.new_zeros(param.size())
                #print()
                #print("No grad Name",name)
                #elif param.grad!=None:# and ~torch.all(param.grad):
                #print("Grad",name)
        #break

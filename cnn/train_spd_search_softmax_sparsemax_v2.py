...
from model_spd_v0_radar_search_softmax_sparsemax_v2 import Network
...
def use_constraint_optimization_as_convex_layer_with_sparsemax(x, n):
    ...
    optimization_problem = cp.Problem(objective, constraint)
    #python interface
    layer = CvxpyLayer(optimization_problem, parameters=[x_], variables=[w_])
    w, = layer(x)
    w_num = torch.exp(-w)
    w_den = torch.sum(torch.exp(-w), dim=1)
    y = w_num / w_den.view(w.shape[0], 1)
    y = sparsemax(y)
    #print(y)
    return y
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print(
            use_constraint_optimization_as_convex_layer_with_sparsemax(
                model.alphas_normal, n))
        print(
            use_constraint_optimization_as_convex_layer_with_sparsemax(
                model.alphas_reduce, n))
        print(
            torch.sum(
                use_constraint_optimization_as_convex_layer_with_sparsemax(
                    model.alphas_normal, n),

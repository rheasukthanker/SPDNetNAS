...
"""
OPS={
 'none_normal': lambda C_in, dim_in,dim_out, stride: Zero_normal(dim_out),
 'WeightedPooling_normal': lambda C_in, dim_in,dim_out, stride: WeightedPooling(C_in),
 'BiMap_0_normal': lambda C_in, dim_in,dim_out, stride: BiMap_0_normal(C_in,dim_out),
 'BiMap_1_normal': lambda C_in, dim_in,dim_out, stride: BiMap_1_normal(C_in,dim_out),
 'BiMap_2_normal': lambda C_in, dim_in,dim_out, stride: BiMap_2_normal(C_in,dim_out),
 'Skip_normal': lambda C_in, dim_in,dim_out, stride: Skip_normal(),
 'none_reduced': lambda C_in, dim_in,dim_out, stride: Zero_reduced(C_in,dim_in,dim_out),
 'AvgPooling2_reduced': lambda C_in, dim_in,dim_out, stride:  AvgPooling_2(dim_in,dim_out),
 'MaxPooling_reduced': lambda C_in, dim_in,dim_out, stride: MaxPooling(dim_in,dim_out),
 'BiMap_1_reduced': lambda C_in, dim_in,dim_out, stride: BiMap_1_reduced(C_in,dim_in,dim_out),
 'BiMap_2_reduced': lambda C_in, dim_in,dim_out, stride: BiMap_2_reduced(C_in,dim_in,dim_out),
 'Skip_reduced': lambda C_in, dim_in,dim_out, stride: Skip_2(C_in,dim_in,dim_out),
}
...
class Zero_reduced(nn.Module):
    ...
    def forward(self,x):
        output=torch.zeros(x.shape[0],self.C_out,self.dim_out,self.dim_out,dtype=torch.float64)
        output_diag=torch.diag(torch.tensor(1).repeat(self.dim_out))
        output=output+output_diag
        return output
...
class WeightedPooling(nn.Module):
    def __init__(self,C_in):
        ...
        self.C_in=C_in
        self.weights=torch.nn.Parameter(data=torch.Tensor(self.C_in,self.C_in),requires_grad=True)
    ...
    def forward(self,x):
        ...
        for i in range(0,self.C_in):
            weighted_channels.append(functional.bary_geom_weightedbatch(x,weights_batched[:,i,:].squeeze(1)))
...
class BiMap_0_normal(nn.Module):
    def __init__(self,C_in,dim_in):
        ...
        self.C_in=C_in
        self.dim_in=dim_in
        self.layers=nn.Sequential(nn_spd.BiMap(self.C_in,self.C_in,self.dim_in,self.dim_in),nn_spd.BatchNormSPD(self.dim_in))
...
class BiMap_1_normal(nn.Module):
    def __init__(self,C_in,dim_in):
        ...
        self.C_in=C_in
        self.dim_in=dim_in
        self.layers=nn.Sequential(nn_spd.BiMap(self.C_in,self.C_in,self.dim_in,self.dim_in),nn_spd.BatchNormSPD(self.dim_in),nn_spd.ReEig())
...
class BiMap_2_normal(nn.Module):
    def __init__(self,C_in,dim_in):
        ...
        self.C_in=C_in
        self.dim_in=dim_in
        self.layers=nn.Sequential(nn_spd.ReEig(), nn_spd.BiMap(self.C_in, self.C_in, self.dim_in, self.dim_in), nn_spd.BatchNormSPD(self.dim_in))
...
class AvgPooling_2(nn.Module):
    def __init__(self, dim_in,dim_out):
        ...
        #self.stride=stride
        if dim_in==93 and dim_out==30:
           self.model = nn.Sequential(nn_spd.LogEig(), nn.AvgPool2d(4, stride=3), nn_spd.ExpEig())
        elif dim_in==93 and dim_out==70:
            self.model = nn.Sequential(nn_spd.LogEig(),torch.nn.AvgPool2d(24,stride=1), nn_spd.ExpEig())
        elif dim_in==70 and dim_out==50:
            self.model = nn.Sequential(nn_spd.LogEig(),torch.nn.AvgPool2d(21,stride=1), nn_spd.ExpEig())
        elif dim_in==60 and dim_out==30:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.AvgPool2d(2, stride=2), nn_spd.ExpEig())
        elif dim_in==50 and dim_out==24:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.AvgPool2d(3, stride=2), nn_spd.ExpEig())
        elif dim_in==46 and dim_out==22:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.AvgPool2d(3, stride=2), nn_spd.ExpEig())
        elif dim_in==93 and dim_out==46:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.AvgPool2d(2, stride=2), nn_spd.ExpEig())
        elif dim_in==35 and dim_out==16:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.AvgPool2d(4, stride=2), nn_spd.ExpEig())
        else:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.AvgPool2d(21, stride=1), nn_spd.ExpEig())
...
class MaxPooling(nn.Module):
    def __init__(self, dim_in,dim_out):
        ...
        if dim_in==93 and dim_out==30:
           self.model = nn.Sequential(nn_spd.LogEig(), nn.MaxPool2d(4, stride=3), nn_spd.ExpEig())
        elif dim_in==93 and dim_out==70:
            self.model = nn.Sequential(nn_spd.LogEig(),torch.nn.MaxPool2d(24,stride=1), nn_spd.ExpEig())
        elif dim_in==70 and dim_out==50:
            self.model = nn.Sequential(nn_spd.LogEig(),torch.nn.MaxPool2d(21,stride=1), nn_spd.ExpEig())
        elif dim_in==60 and dim_out==30:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.MaxPool2d(2, stride=2), nn_spd.ExpEig())
        elif dim_in==50 and dim_out==24:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.MaxPool2d(3, stride=2), nn_spd.ExpEig())
        elif dim_in==93 and dim_out==46:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.MaxPool2d(2, stride=2), nn_spd.ExpEig())
        elif dim_in==46 and dim_out==22:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.MaxPool2d(3, stride=2), nn_spd.ExpEig())
        elif dim_in==35 and dim_out==16:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.MaxPool2d(4, stride=2), nn_spd.ExpEig())
        else:
            self.model = nn.Sequential(nn_spd.LogEig(), torch.nn.MaxPool2d(21, stride=1), nn_spd.ExpEig())
...
class BiMap_0_reduced(nn.Module):
    def __init__(self,C_in,dim_in,dim_out):
        ...
        self.dim_in=dim_in
        self.dim_out=dim_out
        self.model = nn.Sequential(nn_spd.BiMap(self.C_in, self.C_in, self.dim_in, self.dim_out),
...
class BiMap_1_reduced(nn.Module):
    def __init__(self,C_in,dim_in,dim_out):
        ...
        self.dim_out=dim_out
        self.dim_in=dim_in
        self.model = nn.Sequential(nn_spd.ReEig(), nn_spd.BiMap(self.C_in, self.C_in, self.dim_in, self.dim_out),
                                   nn_spd.BatchNormSPD(self.dim_out))
...
class BiMap_2_reduced(nn.Module):
    def __init__(self,C_in,dim_in,dim_out):
        ...
        self.C_in=C_in
        self.dim_out=dim_out
        self.model = nn.Sequential(nn_spd.BiMap(self.C_in,self.C_in,self.dim_in,self.dim_out), nn_spd.BatchNormSPD(self.dim_out),
                                   nn_spd.ReEig())
...
class Skip_2(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dim_out%2==0:
         bimap_1= nn_spd.BiMap(self.C_in, self.C_in, self.dim_in,half_out_dim)(x)
         bimap_2= nn_spd.BiMap(self.C_in, self.C_in, self.dim_in,half_out_dim)(x)
        else:
         bimap_1= nn_spd.BiMap(self.C_in, self.C_in, self.dim_in,half_out_dim+1)(x)
         bimap_2= nn_spd.BiMap(self.C_in, self.C_in, self.dim_in,half_out_dim-1)(x)
        for i in range(self.C_in):  # Vectorize
            Cb_pair = self.concat_pairs(bimap_1[:,i, :, :],bimap_2[:,i,:,:])
...
class WeightedPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        weights_batched = use_softmax_as_convex_layer(
            self.weights,

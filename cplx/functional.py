...
def conv_cplx(X, conv_re, conv_im):
    if (isinstance(X, list)):
        ...
        P_Re = [
            conv_re(xx[0][None, ...]) - conv_im(xx[1][None, ...]) for xx in XX
        ]
        P_Im = [
            conv_im(xx[0][None, ...]) + conv_re(xx[1][None, ...]) for xx in XX
        ]
...
def batch_norm2d_cplx(X, running_mean, running_var, gamma11, gamma12, gamma22,
                      bias, momentum, training):
    N, C, H, W = X.shape
    XX = list(X.split(C // 2, 1))
    X_re = XX[0].transpose(0, 1).contiguous().view(C // 2,
                                                   N * H * W)  #(C//2,NHW)
    X_im = XX[1].transpose(0, 1).contiguous().view(C // 2,
                                                   N * H * W)  #(C//2,NHW)
    X_cplx = th.cat((X_re[:, None, :], X_im[:, None, :]), 1)  #(C//2,2,NHW)
    if (training):
        mu_re = X_re.mean(1)
        mu_im = X_im.mean(1)  #(C//2,)
        mu_cplx = th.cat((mu_re[:, None], mu_im[:, None]), 1)  #(C//2,2)
        var_re = X_re.var(1)
        var_im = X_im.var(1)  #(C//2,)
        cov_imre = ((X_re - mu_re[:, None]) *
                    (X_im - mu_im[:, None])).sum(1) / (N * H * W - 1)  #(C//2,)
        cov_cplx = utils.twobytwo_covmat_from_coeffs(var_re, var_im, cov_imre)
        with th.no_grad():
            running_mean = (
                1 - momentum) * running_mean + momentum * mu_cplx  #(C//2,2)
            running_var = (
                1 - momentum) * running_var + momentum * cov_cplx  #(C//2,2,2)
        ...
        running_var_sqinv = utils.twobytwo_sqinv(running_var[:, 0, 0],
                                                 running_var[:, 1, 1],
                                                 running_var[:, 1, 0])
...
def batch_norm2d_cplx_spd(X, running_mean, running_var, weight, bias, momentum,
                          training):
    N, C, H, W = X.shape
    XX = list(X.split(C // 2, 1))
    X_re = XX[0].transpose(0, 1).contiguous().view(C // 2,
    ...
    if (training):
        ...
        with th.no_grad():
            ...
            running_var = (
                1 - momentum) * running_var + momentum * cov_cplx  #(C//2,2,2)
        cov_cplx_sqinv = (functional_spd.SqminvEig()(
            cov_cplx[:, None, :, :]))[:, 0, :, :]
        # cov_cplx_sqinv=utils.twobytwo_sqinv(var_re,var_im,cov_imre)
        Y = cov_cplx_sqinv.matmul((X_cplx - mu_cplx[:, :, None]))
    else:
        running_var_sqinv = (functional_spd.SqminvEig()(
            running_var[:, None, :, :].double())).float()[:, 0, :, :]
        # running_var_sqinv=utils.twobytwo_sqinv(running_var[:,0,0],running_var[:,1,1],running_var[:,1,0])
        Y = running_var_sqinv.matmul((X_cplx - running_mean[:, :, None]))
    weight = utils.twobytwo_covmat_from_coeffs(
        _gamma11, _gamma22, _gamma12)  ##################### CHANGE
    Z = weight.matmul(Y) + bias[:, :, None]
    return Z.view(C, -1).view(C, N, H, W).transpose(0, 1)
    ...
    if (N_estimates is not None):
        ...
        if (f_re[-1].shape[-1] != self._Nestimates):
            f_re = th.cat(
                f_re[:-1] + (th.cat(
                    (f_re[-1],
                     th.zeros(N, 1, n, self._Nestimates - f_re[-1].shape[-1])),
                    -1), ), 1)
        else:
            f_re = th.cat(f_re, 1)
        ...
        if (f_im[-1].shape[-1] != self._Nestimates):
            f_im = th.cat(
                f_im[:-1] + (th.cat(
                    (f_im[-1],
                     th.zeros(N, 1, n, self._Nestimates - f_im[-1].shape[-1])),
                    -1), ), 1)
        else:
            f_im = th.cat(f_im, 1)
    f_re -= f_re.mean(-1, True)
    f_im -= f_im.mean(-1, True)
    f_re = f_re.double()
    f_im = f_im.double()
    X_Re = ((f_re.matmul(f_re.transpose(-1, -2)) +
             f_im.matmul(f_im.transpose(-1, -2))) / (f.shape[-1] - 1))
    X_Im = ((f_im.matmul(f_re.transpose(-1, -2)) -
             f_re.matmul(f_im.transpose(-1, -2))) / (f.shape[-1] - 1))
    if (reg_mode == 'mle'):
        pass

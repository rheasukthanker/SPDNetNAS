...
class Conv1d_cplx(Conv_cplx):
    ...
    def __init__(self,
        ...
        self._conv_Re = nn.Conv1d(in_channels // 2,
                                  out_channels,
                                  kernel_size,
                                  stride,
                                  bias=bias)
        self._conv_Im = nn.Conv1d(in_channels // 2,
                                  out_channels,
                                  kernel_size,
                                  stride,
                                  bias=bias)
...
class FFT(Conv1d_cplx):
    ...
    def __init__(self,
                 ...
                 stride,
                 bias=False):
        super(__class__, self).__init__(in_channels, out_channels, kernel_size,
                                        stride, bias)
        atoms = signal_utils.fourier_atoms(out_channels, kernel_size).conj()
        atoms_Re = th.from_numpy(
            utils.cplx2bichan(atoms)[:, 0, :][:, None, :]).float()
        atoms_Im = th.from_numpy(
            utils.cplx2bichan(atoms)[:, 1, :][:, None, :]).float()
...
class SplitSignal_cplx(Conv1d_cplx):
    ...
    def __init__(self, in_channels, window_size, hop_length):
        super(__class__, self).__init__(in_channels, window_size, window_size,
                                        hop_length, False)
...
class SplitSignalBlock_cplx(Conv1d_cplx):
    ...
    def forward(self, X):
        XX = [
            functional.split_signal_cplx(X[:, i, :, :], self._conv_Re,
                                         self._conv_Im)[:, None, :, :, :]
...
class Conv2d_cplx(Conv_cplx):
    ...
    def __init__(self,
        ...
        self._conv_Re = nn.Conv2d(in_channels // 2,
                                  out_channels // 2,
                                  kernel_size,
                                  stride,
                                  bias=bias)
        self._conv_Im = nn.Conv2d(in_channels // 2,
                                  out_channels // 2,
                                  kernel_size,
                                  stride,
                                  bias=bias)
        sigma = 2. / (in_channels // 2 + out_channels // 2)
        ampli = numpy.random.rayleigh(sigma,
                                      (out_channels // 2, in_channels // 2,
                                       kernel_size[0], kernel_size[1]))
        phase = numpy.random.uniform(-np.pi, np.pi,
                                     (out_channels // 2, in_channels // 2,
                                      kernel_size[0], kernel_size[1]))
        self._conv_Re.weight.data = nn.Parameter(
            th.Tensor(ampli * np.cos(phase)))
        self._conv_Im.weight.data = nn.Parameter(
            th.Tensor(ampli * np.sin(phase)))
...
class BatchNorm2d_cplxSPD(nn.BatchNorm2d):
    ...
    def __init__(self, in_channels):
        ...
        self.momentum = 0.1
        self.running_mean = th.zeros(in_channels // 2, 2)
        self.running_var = th.eye(2)[None, :, :].repeat(
            in_channels // 2, 1, 1) / (2**.5)
        self.weight_ = nn.ParameterList([
            functional_spd.SPDParameter(th.eye(2) / (2**.5))
            for _ in range(in_channels // 2)
        ])
        self.bias = nn.Parameter(th.zeros(in_channels // 2, 2))

    def forward(self, X):
        return functional.batch_norm2d_cplx_spd(X, self.running_mean,
                                                self.running_var, self.weight_,
                                                self.bias, self.momentum,
                                                self.training)
...
class CovPoolBlock_cplx(nn.Module):
    ...
    def forward(self, f):
        XX = [
            functional.cov_pool_cplx(f[:, i, :, :, :], self._reg_mode,

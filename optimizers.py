...
import torch.optim
import functional as functional_optimized
...
class StiefelBlockOptim():
    ...
    def step(self):
        for W in self.parameters:
            dir_tan = th.cat([
                proj_tanX_stiefel(W.grad[:, :, i, :, :].data,
                                  W[:, :, i, :, :].data)[:, :, None, :, :]
                for i in range(W.shape[2])
            ], 2)
            W_new = th.cat([
                ExpX_stiefel(-self.lr * dir_tan[:, :, i, :, :].data,
                             W[:, :, i, :, :].data)[:, :, None, :, :]
                for i in range(W.shape[2])
            ], 2)
            W.data = W_new
...
class SPDOptim():
    ...
    def step(self):
        for W in self.parameters:
            ...
            W.data = functional_optimized.ExpG(-self.lr * dir_tan.data,
                                               W.data)[0, 0]
...
class MixOptimizer():
    ...
    def __init__(self,
        ...
        self.lr = lr
        self.stiefel_parameters = [
            param for param in parameters
            if param.__class__.__name__ == 'StiefelParameter'
        ]
        self.stiefel_block_parameters = [
            param for param in parameters
            if param.__class__.__name__ == 'StiefelBlockParameter'
        ]
        self.spd_parameters = [
            param for param in parameters
            if param.__class__.__name__ == 'SPDParameter'
        ]
        self.weight_vector_parameters = [
            param for param in parameters
            if param.__class__.__name__ == 'WeightVectorParameter'
        ]
        ...
        self.stiefel_block_optim = StiefelBlockOptim(
            self.stiefel_block_parameters, self.lr)
        self.spd_optim = SPDOptim(self.spd_parameters, self.lr)
        self.weight_vector_optim = WeightVectorOptim(
            self.weight_vector_parameters, self.lr)
...
def gram_schmidt(V):
    ...
    for i in range(1, N):
        ...
        for j in range(i):
            ...
            R[j, i] = W[:, j].dot(V[:, i])
        if (isclose(th.norm(V[:, i] - proj),
                    th.DoubleTensor([0]).to(V.device))):
            W[:, i] = V[:, i] / th.norm(V[:, i])
        else:

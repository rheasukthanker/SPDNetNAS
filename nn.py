...
class BiMap(nn.Module):
    ...
    def __init__(self, ho, hi, ni, no):
        ...
        self._W = functional.StiefelParameter(
            th.empty(ho, hi, ni, no, dtype=dtype, device=device))
        self._ho = ho
        self._hi = hi
        self._ni = ni
        self._no = no
...
class BatchNormSPD(nn.Module):
    ...
    def forward(self, X):
        ...
        if (self.training):
            ...
            X_centered = functional.CongrG(X_batched, self.running_mean,
                                           'neg')  #subtract mean
        X_normalized = functional.CongrG(X_centered, self.weight,
                                         'pos')  # add bias
...
class CovPoolBlock(nn.Module):
    ...
    def forward(self, f):
        ff = [
            functional.cov_pool(f[:, i, :, :], self._reg_mode)[:,
                                                               None, :, :, :]
